...
class MaxoutRNN(nn.Module):
    ...
    def _forward_unpadded(self, x, x_mask):
        ...
        for i in range(2, self.num_layers):
            max_output = torch.max(max_output, outputs[i])

        # Transpose back
        output = max_output.transpose(0, 1)
...
class GatedAttnRNN(nn.Module):
    """Gated Attention-baed RNN.
        Given sequences X and Y, match sequence Y to each element in X.
    """
    def __init__(self, x_size, y_size, hidden_size, dropout_rate=0, cell_type=nn.GRUCell):
        super(GatedAttnRNN, self).__init__()
        self.hidden_size = hidden_size
        self.dropout_rate = dropout_rate
        self.cell = cell_type(x_size + y_size, hidden_size)
        self.linear = nn.Linear(x_size + y_size + hidden_size, hidden_size, bias=False)
        self.weight = nn.Linear(hidden_size, 1, bias=False)
        self.gate = Gate(x_size + y_size)
    ...
    def forward(self, x, y, y_mask):
        ...
        """
        v0 = Variable(torch.zeros([x.size(0), self.hidden_size]))
        if y.data.is_cuda:
            v0 = v0.cuda()
        output = [v0.unsqueeze(1)]
        x_len = x.size(1)
        for i in range(x_len):
            u = x[:, i, :]
            v = output[-1]
            s = F.tanh(self.linear(torch.cat([y, u.unsqueeze(1).repeat(1, y.size(1), 1), v.repeat(1, y.size(1), 1)], 2)))
            w_s = self.weight(s).squeeze(2)
            w_s.data.masked_fill_(y_mask.data, -float('inf'))
            a = F.softmax(w_s)
            c = a.unsqueeze(1).bmm(y).squeeze(1)
            u_c = self.gate(torch.cat([u, c], 1))
            v_t = self.cell(u_c, v.squeeze(1))
            output.append(v_t.unsqueeze(1))
        res = torch.cat(output[1:], 1)
        return F.dropout(res, p=self.dropout_rate, training=self.training)
...
class StochasticAnsPointer(nn.Module):
    def __init__(self, x_size, y_size, hidden_size, hop=1, dropout_rate=0, normalize=True):
        super(StochasticAnsPointer, self).__init__()
        """"StochasticAnsPointer
        """
        self.normalize = normalize
        self.hidden_size = hidden_size
        self.hop = hop
        self.dropout_rate = dropout_rate
        # self.self_attn = LinearSeqAttn(y_size)
        self.self_attn = NonLinearSeqAttn(y_size, hidden_size)
        self.FFNs_start = nn.ModuleList()
        self.SFUs_start = nn.ModuleList()
        self.FFNs_end = nn.ModuleList()
        ...
        """Bilinear Term
        """
        self.self_attn_linear = LinearSeqAttn(y_size)
        self.start_attn_liner = nn.Linear(y_size, x_size)
        self.end_attn_liner = nn.Linear(y_size, x_size)

    def forward(self, x, y, x_mask, y_mask):
        """"MemoryAnsPointer
        """
        attn = self.self_attn(y, y_mask)
        z_s = attn.unsqueeze(1).bmm(y) # [B, 1, I]
        z_e = None
        s = None
        e = None
        p_s = None
        ...
        """Bilinear Term
        """
        weights = self.self_attn_linear(y, y_mask)
        attn = weights.unsqueeze(1).bmm(y).squeeze(1)
        start_Wy = self.start_attn_liner(attn)
        start_xWy = x.bmm(start_Wy.unsqueeze(2)).squeeze(2)
        start_xWy.data.masked_fill_(x_mask.data, -float('inf'))
        end_Wy = self.end_attn_liner(attn)
        end_xWy = x.bmm(end_Wy.unsqueeze(2)).squeeze(2)
        end_xWy.data.masked_fill_(x_mask.data, -float('inf'))

        # Merge two results
        s_list_ = [start_xWy.unsqueeze(2)]
        e_list_ = [end_xWy.unsqueeze(2)]
        if self.training:
            # In training we output log-softmax for NLL
            for s in s_list:
                if random.random() < self.dropout_rate:
                    continue
                s_list_.append(s.unsqueeze(2))
            for e in e_list:
                if random.random() < self.dropout_rate:
                    continue
                e_list_.append(e.unsqueeze(2))
            s = torch.cat(s_list_, 2).mean(2)
            e = torch.cat(e_list_, 2).mean(2)
