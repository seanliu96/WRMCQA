...
"""Implementation of the Stochastic Mnemonic Reader."""
...
class StochasticMnemonicReader(nn.Module):
    RNN_TYPES = {'lstm': nn.LSTM, 'gru': nn.GRU, 'rnn': nn.RNN}
    CELL_TYPES = {'lstm': nn.LSTMCell, 'gru': nn.GRUCell, 'rnn': nn.RNNCell}
    def __init__(self, args, normalize=True):
        super(StochasticMnemonicReader, self).__init__()
        # Store config
        self.args = args
        ...
        c_input_size = args.embedding_dim + args.char_hidden_size * 2 + args.num_features
        q_input_size = args.embedding_dim + args.char_hidden_size * 2 + args.num_features

        # Encoder
        self.encoder_rnn = layers.MaxoutRNN(
        ...
        for i in range(args.hop):
            # interactive aligner
            self.interactive_aligners.append(layers.SeqAttnMatch(c_hidden_size, identity=True))
            self.interactive_SFUs.append(layers.SFU(c_hidden_size, 3 * c_hidden_size))
            # self aligner
            self.self_aligners.append(layers.SelfAttnMatch(c_hidden_size, identity=True, diag=False))
            self.self_SFUs.append(layers.SFU(c_hidden_size, 3 * c_hidden_size))
            # aggregating
        ...
        # Stochastic Answer Pointer Network
        self.ans_ptr = layers.StochasticAnsPointer(
            x_size=c_hidden_size, 
            y_size=q_hidden_size, 
            hidden_size=args.hidden_size, 
            hop=args.hop,
            dropout_rate=args.dropout_rnn,
            normalize=normalize
    ...
    def forward(self, x1, x1_c, x1_f, x1_mask, x2, x2_c, x2_f, x2_mask):
        ...
        c = self.encoder_rnn(torch.cat(crnn_input, 2), x1_mask)
        
        # Encode question with RNN
        q = self.encoder_rnn(torch.cat(qrnn_input, 2), x2_mask)

        # Align and aggregate
        c_check = c
        for i in range(self.args.hop):
            q_tilde = self.interactive_aligners[i](c_check, q, x2_mask)
            c_bar = self.interactive_SFUs[i](c_check, torch.cat([q_tilde, c_check * q_tilde, c_check - q_tilde], 2))
            c_tilde = self.self_aligners[i](c_bar, x1_mask)
            c_hat = self.self_SFUs[i](c_bar, torch.cat([c_tilde, c_bar * c_tilde, c_bar - c_tilde], 2))
            c_check = self.aggregate_rnns[i](c_hat, x1_mask)

        # Predict
        start_scores, end_scores = self.ans_ptr(c_check, q, x1_mask, x2_mask)
