...
class WRMCQA(object):
    ...
    def __init__(
        ...
        Args:
            ...
            char_embedding_file: if given, will expand DocReader char dictionary to use
              all available pretrained embeddings.
        ...
        if char_embedding_file:
            logger.info('Expanding char dictionary...')
            chars = reader.utils.index_embedding_chars(char_embedding_file)
            added = self.reader.expand_char_dictionary(chars)
            self.reader.load_char_embeddings(added, char_embedding_file)
        if cuda:
            self.reader.cuda()
        if data_parallel:
    ...
    def process_batch(self, queries, candidates=None, top_n=1, n_docs=5,
            ...
            for rel_didx, did in enumerate(all_docids[qidx]):
                ...
                for sidx in range(start, end):
                    if (len(q_tokens[qidx].words()) > 0 and
                            len(c_tokens[sidx].words()) > 0):
                        examples.append({
                            'id': (qidx, rel_didx, sidx),
                            'question': q_tokens[qidx].words(),
                            'question_char': q_tokens[qidx].chars(),
                            'qlemma': q_tokens[qidx].lemmas(),
                            'qpos': q_tokens[qidx].pos(),
                            'qner': q_tokens[qidx].entities(),
                            'document': c_tokens[sidx].words(),
                            'document_char': c_tokens[sidx].chars(),
                            'clemma': c_tokens[sidx].lemmas(),
                            'cpos': c_tokens[sidx].pos(),
                            'cner': c_tokens[sidx].entities(),
                        })
        ...
        for batch in self._get_loader(examples, num_loaders):
            ...
            result_handles.append((handle, ex_ids, batch[0].size(0)))
        ...
        for queue in queues:
            ...
            doc_span_score = defaultdict(float)
            doc_count = defaultdict(int)
            span_count = defaultdict(int)
            if self.use_ala:
                for i in range(len(queue)):
                    score, (qidx, rel_didx, sidx), s, e = queue[i]
                    prediction = {
                        'doc_id': all_docids[qidx][rel_didx],
                        'span': c_tokens[sidx].slice(s, e + 1).untokenize(),
                        'doc_score': float(all_doc_scores[qidx][rel_didx]),
                        'paragraph_len': float(len(c_tokens[sidx])),
                        'span_score': float(score),
                    }
                    if return_context:
                        prediction['context'] = {
                            'text': c_tokens[sidx].untokenize(),
                            'start': c_tokens[sidx].offsets()[s][0],
                            'end': c_tokens[sidx].offsets()[e][1],
                        }
                    doc_span_score[prediction['doc_id']] += prediction['span_score']
                    doc_count[prediction['doc_id']] += 1
                    span_count[prediction['span']] += 1
                    predictions.append(prediction)
                for prediction in predictions:
                    overall_score[prediction['span']] += prediction['span_score'] * prediction['doc_score']
                for prediction in predictions:
                    prediction['overall_score'] = overall_score[prediction['span']] + prediction['span_score'] * prediction['doc_score']
                predictions.sort(key=itemgetter('overall_score', 'span_score'), reverse=True)
